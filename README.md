...
| 键 | 行为 |
|---|---|
| 字母 / `'` | 组成编码（`'` 为音节分隔；`,` 先上屏字面逗号，紧跟小写字母且逗号前是空白/换行/文首才转动态词） |
...
**紧跟**小写字母才删掉逗号进选择菜单——且仅当逗号前是空格/tab/换行
（或文首）才武装，中文「，」后接拼音、英文 `hello,world` 都不会误触发
（`,check` → ✅、`,date` → 日期、`eval:` 值执行 JS）；
跟任何其他键（空格/数字/标点/方向键）都无事发生——正常打 `hello, world` 全程无感。
模式内空格/回车 = 精确 > 首选 > 字面逗号。
